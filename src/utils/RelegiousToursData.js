const tours = [
    {
        "id": "kedarnath-yatra",
        "title": "Kedarnath Yatra",
        "subtitle": "Pilgrimage to the Abode of Lord Shiva",
        "image": "/Kedarnath.jpg",
        "description": "Kedarnath Yatra – 3 Days / 2 Nights\nRoute: Haridwar → Guptkashi / Sitapur → Kedarnath → Haridwar\nBest Time: May to October",
        "highlights": [
            "Darshan at the sacred Kedarnath Temple – one of the 12 Jyotirlingas and part of Char Dham.",
            "Trek through scenic Himalayan routes or opt for helicopter service.",
            "Visit Guptkashi, Triyuginarayan, and Sonprayag.",
            "Witness the Mandakini River and serene mountain landscapes."
        ],
        "itinerary": [
            {
                "day": 1,
                "title": "Haridwar – Guptkashi / Sitapur",
                "description": "Haridwar – Guptkashi / Sitapur (210–220 km / 8–9 hrs)\nEarly morning departure from Haridwar\nScenic drive along the Ganga and Alaknanda rivers\nEn route stops at Devprayag, Rudraprayag\nReach Guptkashi or Sitapur by evening\nVisit Ardh Narishwar Temple if time permits\nOvernight Stay: Guptkashi / Sitapur"
            },
            {
                "day": 2,
                "title": "Guptkashi / Sitapur – Kedarnath",
                "description": "Guptkashi / Sitapur – Kedarnath (Drive + 18 km Trek / 5–6 hrs)\nEarly morning drive to Sonprayag, transfer to Gaurikund\nBegin 18 km trek to Kedarnath (Pony/Palki/Helicopter option available)\nReach Kedarnath, visit Kedarnath Temple and surrounding sites\nAttend the evening aarti if time allows\nOvernight Stay: Kedarnath (basic guest house or tented accommodation)"
            },
            {
                "day": 3,
                "title": "Kedarnath – Guptkashi – Haridwar",
                "description": "Kedarnath – Guptkashi – Haridwar (Trek + Drive / 10–11 hrs)\nEarly morning darshan (if missed earlier), then trek down to Gaurikund\nReturn via Sonprayag to Guptkashi, then drive back to Haridwar\nReach Haridwar by late evening\nTour concludes with divine blessings"
            }
        ],
        "inclusions": [
            "Transportation from Haridwar to Haridwar by private vehicle.",
            "Accommodation in Guptkashi/Sitapur and Kedarnath (on sharing basis).",
            "Breakfast and dinner during the journey.",
            "Parking, driver charges, tolls, fuel, and basic coordination."
        ],
        "exclusions": [
            "Helicopter, pony, or palki charges.",
            "Personal expenses, snacks, tips.",
            "Medical or emergency evacuation charges.",
            "Entry fees, VIP darshan, or special pooja.",
            "Anything not mentioned under 'Inclusions'."
        ],
        "things_to_carry": [],
        "best_time_to_visit": [],
        "why_choose_us": [
            "Local experts with extensive Yatra planning experience.",
            "Clean, hygienic stays near temples and base camps.",
            "Trekking/transport assistance for elderly & first-time yatris.",
            "Transparent pricing, personalized service.",
            "24x7 assistance during the yatra."
        ]
    },
    {
        "id": "adi-kailash",
        "title": "Adi Kailash & Om Parvat Yatra",
        "subtitle": "The Himalayan Mystery",
        "image": "/AdiKailash.webp",
        "description": "Adi Kailash & Om Parvat Yatra (9 Days / 8 Nights)\nRoute: Kathgodam → Pithoragarh → Dharchula → Gunji → Adi Kailash → Om Parvat → Kathgodam\nRegion: Kumaon Himalayas, Uttarakhand\nMax Altitude: 15,500 ft approx.",
        "highlights": [
            "Darshan of Adi Kailash, known as the Chhota Kailash, sacred to Lord Shiva",
            "Visit the divine Om Parvat, where the natural formation of \"ॐ\" is seen on the mountain",
            "Spiritual experiences at sacred sites like Parvati Sarovar, Kunti Parvat, and Brahma Parvat",
            "Adventure drive through high mountain passes like Lipulekh and beautiful valleys",
            "Cultural insights at tribal villages and monasteries near the Indo-Tibet border"
        ],
        "itinerary": [
            {
                "day": 1,
                "title": "Kathgodam – Pithoragarh",
                "description": "Kathgodam – Pithoragarh (250 km / 8-9 hrs)\nEarly morning arrival at Kathgodam and group departure\nScenic drive through Almora, Berinag, and Askot\nReach Pithoragarh and check into hotel\nOvernight stay: Pithoragarh"
            },
            {
                "day": 2,
                "title": "Pithoragarh – Dharchula",
                "description": "Pithoragarh – Dharchula (90 km / 3-4 hrs)\nDrive along Kali River, passing beautiful Himalayan landscapes\nComplete ILP (Inner Line Permit) process in Dharchula\nFree time to explore the Indo-Nepal border town\nOvernight stay: Dharchula"
            },
            {
                "day": 3,
                "title": "Dharchula – Nabi / Gunji",
                "description": "Dharchula – Nabi / Gunji (80 km / 5-6 hrs)\nEnter the restricted zone with government-permitted vehicles\nDrive via Chialekh and Garbyang – ancient high-altitude villages\nReach Gunji or Nabi and acclimatize\nOvernight stay: Homestay/Camp in Gunji/Nabi"
            },
            {
                "day": 4,
                "title": "Gunji – Kuti – Jyolingkong – Adi Kailash – Gunji",
                "description": "Gunji – Kuti – Jyolingkong – Adi Kailash – Gunji (100 km round trip)\nStart early for Jyolingkong (the base of Adi Kailash)\nEn route visit Kunti Parvat and Pandav Parvat\nEnjoy darshan of Adi Kailash and Parvati Sarovar\nPerform puja and return to Gunji\nOvernight stay: Gunji/Nabi"
            },
            {
                "day": 5,
                "title": "Gunji – Om Parvat",
                "description": "Gunji – Om Parvat – Kalapani – Gunji (70 km round trip)\nMorning drive to see Om Parvat, where natural snow patterns form the sacred ॐ symbol\nVisit Kalapani Temple, origin of River Kali\nExplore Nabidhang and enjoy views of Brahma Parvat\nReturn to Gunji by evening\nOvernight stay: Gunji/Nabi"
            },
            {
                "day": 6,
                "title": "Gunji – Dharchula",
                "description": "Gunji – Dharchula\nBegin return journey towards Dharchula\nEn route enjoy local Kumaoni food and village interactions\nOvernight stay: Dharchula"
            },
            {
                "day": 7,
                "title": "Dharchula – Pithoragarh",
                "description": "Dharchula – Pithoragarh\nLeisurely drive back to Pithoragarh\nVisit local temples and enjoy the Himalayan views\nOvernight stay: Pithoragarh"
            },
            {
                "day": 8,
                "title": "Pithoragarh – Kathgodam",
                "description": "Pithoragarh – Kathgodam\nFinal journey back to Kathgodam with drop by evening\nYatra concludes with divine blessings and memorable experiences"
            }
        ],
        "inclusions": [
            "Accommodation (hotels, homestays, camps) on twin/triple sharing basis.",
            "Vegetarian meals (Breakfast, Lunch, Dinner).",
            "Local transportation from Kathgodam and back in tempo traveller/jeep.",
            "Inner Line Permits and government formalities.",
            "English/Hindi speaking experienced guide.",
            "First-aid and oxygen cylinder support."
        ],
        "exclusions": [
            "Travel to/from Kathgodam.",
            "Personal expenses like shopping, phone usage, bottled water.",
            "Tips, extra meals, and entry tickets not mentioned.",
            "Medical insurance and emergency evacuation charges."
        ],
        "things_to_carry": [
            "Government-issued photo ID (Aadhaar/Passport).",
            "Warm jackets, gloves, thermals, trekking shoes.",
            "Sunglasses, sunscreen, and personal toiletries.",
            "Reusable water bottle, power bank, and basic medicines."
        ],
        "best_time_to_visit": [
            "May to mid-July and September to early October.",
            "Avoid monsoon season due to landslides and bad roads."
        ],
        "why_choose_us": [
            "Trusted spiritual and adventure travel partner.",
            "End-to-end assistance including permits and local coordination.",
            "Culturally immersive journeys with experienced Kumaon guides.",
            "Well-planned itineraries for altitude adaptation and comfort.",
            "Small-group departures with personalized attention."
        ]
    },
    {
        "id": "char-dham-yatra",
        "title": "Char Dham Yatra",
        "subtitle": "Yamunotri, Gangotri, Kedarnath & Badrinath Pilgrimage",
        "image": "/CharDham.webp",
        "description": "Char Dham Yatra from Haridwar (11 Days / 10 Nights)\nRoute: Haridwar → Barkot → Yamunotri → Uttarkashi → Gangotri → Guptkashi → Kedarnath → Badrinath → Haridwar\nRegion: Garhwal Himalayas, Uttarakhand",
        "highlights": [
            "Visit the four sacred Dhams: Yamunotri, Gangotri, Kedarnath, and Badrinath",
            "Take a dip in the holy rivers: Yamuna, Ganga, Mandakini, and Alaknanda",
            "Journey through scenic landscapes, snow-capped peaks, and serene valleys",
            "Experience ancient temples and rituals at high altitudes",
            "Attend evening Ganga Aarti in Haridwar and visit Devprayag Sangam"
        ],
        "itinerary": [
            {
                "day": 1,
                "title": "Haridwar – Barkot",
                "description": "Haridwar – Barkot (180 km / 7-8 hrs)\nPick-up from Haridwar in the morning\nDrive along the scenic Yamuna valley via Mussoorie and Kempty Falls\nArrive in Barkot and check in to your hotel\nOvernight stay: Barkot"
            },
            {
                "day": 2,
                "title": "Barkot – Yamunotri – Barkot",
                "description": "Barkot – Yamunotri – Barkot (36 km drive + 6 km trek one way)\nEarly drive to Janki Chatti\nBegin the 6 km trek to Yamunotri Temple\nTake darshan and visit nearby hot springs\nReturn trek and drive back to Barkot\nOvernight stay: Barkot"
            },
            {
                "day": 3,
                "title": "Barkot – Uttarkashi",
                "description": "Barkot – Uttarkashi (100 km / 4 hrs)\nDrive to Uttarkashi along scenic pine forest routes\nEn route visit Prakateshwar Mahadev Cave\nVisit the Kashi Vishwanath Temple in Uttarkashi\nOvernight stay: Uttarkashi"
            },
            {
                "day": 4,
                "title": "Uttarkashi – Gangotri – Uttarkashi",
                "description": "Uttarkashi – Gangotri – Uttarkashi (100 km each side / 3-4 hrs)\nDrive to Gangotri Temple, situated near the origin of the River Ganga\nTake a holy dip in Bhagirathi River, explore Pandava Gufa and Surya Kund\nReturn drive to Uttarkashi\nOvernight stay: Uttarkashi"
            },
            {
                "day": 5,
                "title": "Uttarkashi – Guptkashi",
                "description": "Uttarkashi – Guptkashi (220 km / 8-9 hrs)\nLong scenic drive via Tehri Dam and Rudraprayag\nWitness the confluence of Alaknanda and Mandakini Rivers\nArrive at Guptkashi by evening\nOvernight stay: Guptkashi"
            },
            {
                "day": 6,
                "title": "Guptkashi – Gaurikund – Kedarnath",
                "description": "Guptkashi – Gaurikund – Kedarnath (30 km drive + 16 km trek)\nEarly morning drive to Gaurikund\nBegin the trek or take a pony/palki/helicopter to Kedarnath\nVisit the Kedarnath Temple and attend evening aarti\nOvernight stay: Kedarnath"
            },
            {
                "day": 7,
                "title": "Kedarnath – Gaurikund – Guptkashi",
                "description": "Kedarnath – Gaurikund – Guptkashi\nMorning darshan at the temple\nTrek back to Gaurikund and drive to Guptkashi\nOvernight stay: Guptkashi"
            },
            {
                "day": 8,
                "title": "Guptkashi – Badrinath",
                "description": "Guptkashi – Badrinath (190 km / 8-9 hrs)\nScenic drive via Joshimath and Vishnuprayag\nVisit the Badrinath Temple and take a dip in Tapt Kund\nAttend evening aarti and explore the temple surroundings\nOvernight stay: Badrinath"
            },
            {
                "day": 9,
                "title": "Badrinath – Rudraprayag",
                "description": "Badrinath – Rudraprayag (160 km / 6-7 hrs)\nMorning darshan at Badrinath\nVisit Mana Village, the last village before the Indo-Tibet border\nDrive to Rudraprayag via Karanprayag\nOvernight stay: Rudraprayag"
            },
            {
                "day": 10,
                "title": "Rudraprayag – Haridwar",
                "description": "Rudraprayag – Haridwar (165 km / 6 hrs)\nDrive back to Haridwar\nEn route visit Devprayag Sangam\nReach Haridwar by evening and conclude the yatra\nTour ends with blessings and memories"
            }
        ],
        "inclusions": [
            "Transportation from Haridwar to Haridwar (vehicle as per group size).",
            "Accommodation in hotels and guesthouses on double/triple sharing basis.",
            "Breakfast and dinner throughout the tour.",
            "Assistance for temple visits and trek points.",
            "Local tour guide and support team."
        ],
        "exclusions": [
            "Helicopter, pony, or palki charges.",
            "Entry fees at any monuments or hot springs.",
            "Personal expenses such as laundry, phone calls, and snacks.",
            "Travel insurance and emergency evacuation costs.",
            "Anything not mentioned under 'Inclusions'."
        ],
        "things_to_carry": [
            "Government-issued ID proof.",
            "Warm clothes, gloves, woollen caps, socks.",
            "Raincoat or poncho.",
            "Trekking or walking shoes with good grip.",
            "Personal medication and basic first aid.",
            "Torchlight, reusable water bottle, and light snacks."
        ],
        "best_time_to_visit": [
            "May to June: Post-winter opening of temples, pleasant weather.",
            "September to October: Clear skies, less crowded, post-monsoon freshness."
        ],
        "why_choose_us": [
            "Expertly crafted spiritual and adventure experiences.",
            "Experienced local guides and on-ground support.",
            "Comfortable stays and hygienic meals.",
            "Transparent pricing and personalized attention.",
            "Dedicated assistance before and during the trip."
        ]
    },
    {
        "id": "do-dham-yatra",
        "title": "Do Dham Yatra",
        "subtitle": "Kedarnath & Badrinath Pilgrimage",
        "image": "/DoDham.webp",
        "description": "Do Dham Yatra – Kedarnath & Badrinath (6 Days / 5 Nights)",
        "highlights": [
            "Seek blessings at the revered Kedarnath and Badrinath Temples.",
            "Experience the majestic beauty of the Garhwal Himalayas.",
            "Trek to Kedarnath or opt for helicopter/palki services.",
            "Take a dip in the sacred Tapt Kund and explore Mana Village.",
            "Attend soul-stirring evening aartis and witness river confluences."
        ],
        "itinerary": [
            {
                "day": 1,
                "title": "Haridwar – Guptkashi",
                "description": "Haridwar – Guptkashi (220 km / 8-9 hrs)\nMorning departure from Haridwar\nDrive along the scenic routes passing Devprayag and Rudraprayag\nEn route view the confluence of Alaknanda and Mandakini rivers\nArrive at Guptkashi and check into hotel\nOvernight stay: Guptkashi"
            },
            {
                "day": 2,
                "title": "Guptkashi – Gaurikund – Kedarnath",
                "description": "Guptkashi – Gaurikund – Kedarnath (30 km drive + 16 km trek)\nEarly morning drive to Gaurikund\nBegin the trek or opt for pony/palki/helicopter\nReach Kedarnath Temple and perform evening darshan\nOvernight stay: Kedarnath (in guesthouse/camp)"
            },
            {
                "day": 3,
                "title": "Kedarnath – Gaurikund – Guptkashi",
                "description": "Kedarnath – Gaurikund – Guptkashi\nMorning darshan at Kedarnath Temple\nTrek back to Gaurikund and drive to Guptkashi\nOvernight stay: Guptkashi"
            },
            {
                "day": 4,
                "title": "Guptkashi – Badrinath",
                "description": "Guptkashi – Badrinath (190 km / 8-9 hrs)\nScenic drive via Joshimath, Vishnuprayag, and mountain valleys\nArrival at Badrinath; take a dip in Tapt Kund\nVisit the Badrinath Temple and attend the evening aarti\nOvernight stay: Badrinath"
            },
            {
                "day": 5,
                "title": "Badrinath – Rudraprayag",
                "description": "Badrinath – Rudraprayag (160 km / 6-7 hrs)\nEarly morning darshan at Badrinath Temple\nExplore Mana Village – Bhim Pul, Vyas Gufa, Saraswati River\nDrive to Rudraprayag via Karanprayag\nOvernight stay: Rudraprayag"
            },
            {
                "day": 6,
                "title": "Rudraprayag – Haridwar",
                "description": "Rudraprayag – Haridwar (165 km / 6 hrs)\nDrive back to Haridwar\nEn route visit Devprayag – confluence of Bhagirathi and Alaknanda Rivers\nArrive in Haridwar by evening\nYatra concludes with blessings and memories"
            }
        ],
        "inclusions": [
            "Transportation from Haridwar to Haridwar.",
            "Hotel and guesthouse accommodations on twin/triple sharing basis.",
            "Breakfast and dinner during the trip.",
            "Assistance with Kedarnath trek logistics.",
            "Experienced guide and support staff."
        ],
        "exclusions": [
            "Helicopter, pony, or palki charges for Kedarnath.",
            "Entry tickets, personal expenses, and tips.",
            "Travel insurance and emergency evacuation.",
            "Anything not mentioned under inclusions."
        ],
        "things_to_carry": [
            "Valid photo ID proof.",
            "Warm clothes, raincoat, and sturdy trekking shoes.",
            "Personal medicines and toiletries.",
            "Reusable water bottle and torchlight.",
            "Energy snacks for the trek."
        ],
        "best_time_to_visit": [
            "May to June: Clear weather and full access to temples.",
            "September to October: Crisp post-monsoon air and fewer pilgrims."
        ],
        "why_choose_us": [
            "Dedicated spiritual travel specialists.",
            "Clean stays and nutritious meals at every stop.",
            "Friendly, professional team with 24/7 assistance.",
            "Tailored packages for families, seniors, and groups.",
            "Trusted logistics with local expertise."
        ]
    },
    {
        "id": "panch-kedar",
        "title": "Panch Kedar Yatra",
        "subtitle": "Five Ancient Shiva Temples",
        "image": "/PanchKedar.webp",
        "description": "Panch Kedar Yatra (Haridwar to Haridwar, 12 Days / 11 Nights)\nRegion: Garhwal Himalayas, Uttarakhand\nMaximum Altitude: 12,073 ft (Tungnath)\nBest Time: May–October\nDifficulty: Moderate",
        "highlights": [
            "Visit all five Panch Kedar temples – Kedarnath, Tungnath, Madhyamaheshwar, Rudranath, and Kalpeshwar",
            "Experience sacred Himalayan trails – Trek through forests, meadows, and ancient villages",
            "Stay in scenic locations – Chopta, Madhyamaheshwar, Rudranath, and Joshimath",
            "Spiritual and cultural immersion – Witness daily temple rituals and Garhwali traditions"
        ],
        "itinerary": [
            {
                "day": 1,
                "title": "Haridwar to Guptkashi",
                "description": "Haridwar to Guptkashi (Drive – 210 km / 8–9 hrs)\nDepart early from Haridwar.\nScenic drive along Rishikesh, Devprayag, Rudraprayag, and Mandakini River.\nReach Guptkashi, the base for Kedarnath Yatra.\nOvernight Stay: Guesthouse/hotel in Guptkashi."
            },
            {
                "day": 2,
                "title": "Guptkashi to Gaurikund to Kedarnath",
                "description": "Guptkashi to Gaurikund to Kedarnath (Drive – 30 km / 1 hr + Trek – 18 km / 7–8 hrs)\nDrive to Gaurikund (starting point of Kedarnath trek).\nTrek to Kedarnath Temple (11,755 ft).\nAttend evening aarti at the temple.\nOvernight Stay: Guesthouse/dharamshala in Kedarnath."
            },
            {
                "day": 3,
                "title": "Kedarnath to Guptkashi",
                "description": "Kedarnath to Guptkashi (Trek – 18 km / 6–7 hrs + Drive – 30 km / 1 hr)\nEarly morning darshan at Kedarnath Temple.\nTrek back to Gaurikund, followed by a drive to Guptkashi.\nOvernight Stay: Guesthouse/hotel in Guptkashi."
            },
            {
                "day": 4,
                "title": "Guptkashi to Chopta",
                "description": "Guptkashi to Chopta (Drive – 50 km / 3 hrs)\nDrive through forests and meadows to Chopta (8,790 ft).\nLeisure time for acclimatization.\nOvernight Stay: Campsite/homestay in Chopta."
            },
            {
                "day": 5,
                "title": "Chopta to Tungnath & Chandrashila, back to Ukhimath",
                "description": "Chopta to Tungnath & Chandrashila, back to Ukhimath (Trek – 5 km one way / 5 hrs, Drive – 30 km / 1.5 hrs)\nEarly morning trek to Tungnath Temple (12,073 ft) – the highest Shiva temple.\nOptional 1 km trek to Chandrashila Peak (12,083 ft) for Himalayan views.\nTrek back to Chopta and drive to Ukhimath.\nOvernight Stay: Guesthouse/homestay in Ukhimath."
            },
            {
                "day": 6,
                "title": "Ukhimath to Ransi to Madhyamaheshwar",
                "description": "Ukhimath to Ransi to Madhyamaheshwar (Drive – 60 km / 3 hrs + Trek – 19 km / 7–8 hrs)\nDrive to Ransi, the base for Madhyamaheshwar trek.\nTrek through dense forests and scenic villages to Madhyamaheshwar Temple (11,450 ft).\nOvernight Stay: Guesthouse/homestay in Madhyamaheshwar."
            },
            {
                "day": 7,
                "title": "Madhyamaheshwar to Ransi, Drive to Sagar Village",
                "description": "Madhyamaheshwar to Ransi, Drive to Sagar Village (Trek – 19 km / 6 hrs + Drive – 60 km / 3 hrs)\nMorning darshan at the temple.\nTrek back to Ransi and drive to Sagar Village (base for Rudranath trek).\nOvernight Stay: Homestay in Sagar Village."
            },
            {
                "day": 8,
                "title": "Sagar to Panar Bugyal",
                "description": "Sagar to Panar Bugyal (Trek – 10 km / 6 hrs)\nTrek through oak and rhododendron forests to Panar Bugyal (11,155 ft).\nOvernight Stay: Campsite at Panar Bugyal."
            },
            {
                "day": 9,
                "title": "Panar Bugyal to Rudranath Temple, back to Panar",
                "description": "Panar Bugyal to Rudranath Temple, back to Panar (Trek – 12 km round trip / 6 hrs)\nTrek to Rudranath Temple (11,800 ft) – the most remote Panch Kedar.\nExplore Nanda Devi & Trishul views from the temple.\nReturn to Panar Bugyal.\nOvernight Stay: Campsite at Panar Bugyal."
            },
            {
                "day": 10,
                "title": "Panar Bugyal to Sagar, Drive to Urgam",
                "description": "Panar Bugyal to Sagar, Drive to Urgam (Trek – 10 km / 4–5 hrs + Drive – 100 km / 5 hrs)\nTrek back to Sagar Village.\nDrive to Urgam Valley, the base for Kalpeshwar Temple.\nOvernight Stay: Guesthouse in Urgam."
            },
            {
                "day": 11,
                "title": "Urgam to Kalpeshwar Temple, Drive to Joshimath",
                "description": "Urgam to Kalpeshwar Temple, Drive to Joshimath (Trek – 2 km round trip + Drive – 50 km / 3 hrs)\nShort trek to Kalpeshwar Temple (7,200 ft), where Lord Shiva’s matted hair (Jata) is worshipped.\nDrive to Joshimath.\nOvernight Stay: Hotel in Joshimath."
            },
            {
                "day": 12,
                "title": "Joshimath to Haridwar",
                "description": "Joshimath to Haridwar (Drive – 270 km / 10–11 hrs)\nEarly morning departure.\nScenic drive via Rudraprayag & Devprayag.\nReach Haridwar by evening.\nTrip Ends."
            }
        ],
        "inclusions": [
            "Accommodation – Guesthouses, camps, and homestays on a twin/triple-sharing basis.",
            "Meals – Nutritious vegetarian meals from Day 1 dinner to Day 12 breakfast.",
            "Transportation – Private vehicle from Haridwar to Haridwar.",
            "Experienced Trek Leader & Local Guides.",
            "All Permits & Entry Fees.",
            "First Aid & Oxygen Cylinder (in high-altitude areas)."
        ],
        "exclusions": [
            "Any personal expenses (laundry, tips, etc.).",
            "Emergency evacuation or rescue due to weather or medical conditions.",
            "Insurance of any kind.",
            "Porter/mule charges for personal luggage.",
            "Anything not mentioned in the inclusions."
        ],
        "things_to_carry": [],
        "best_time_to_visit": [
            "May–October"
        ],
        "why_choose_us": [
            "Experienced Trek Guides – Local experts for an insightful and safe journey.",
            "Comfortable Stay & Nutritious Meals – Authentic homestays, camps, and dharamshalas.",
            "Authentic Spiritual Experience – Visit remote temples, explore Garhwali culture.",
            "Seamless Travel Support – End-to-end assistance from permits to accommodation.",
            "Small Groups for a Personalized Experience – Avoid the crowds and connect deeply with nature."
        ]
    },
    // {
    //     "id": "badrinath-dham",
    //     "title": "Badrinath Yatra",
    //     "subtitle": "Sacred Journey to Badrinath Temple",
    //     "image": "/Badrinath.jpg",
    //     "description": "Badrinath Yatra – 3 Days / 2 Nights\nRoute: Haridwar → Joshimath → Badrinath → Haridwar",
    //     "highlights": [
    //         "Darshan at Badrinath Temple, one of the four Char Dhams.",
    //         "Scenic drive through Devprayag, Rudraprayag, Karnaprayag.",
    //         "Explore Mana Village – the last village of India.",
    //         "Visit holy sites like Tapt Kund, Narad Kund, and witness the confluence of rivers."
    //     ],
    //     "itinerary": [
    //         {
    //             "day": 1,
    //             "title": "Haridwar – Joshimath / Pipalkoti",
    //             "description": "Haridwar – Joshimath / Pipalkoti (260–280 km / 9–10 hrs)\nEarly departure from Haridwar\nDrive along the Alaknanda River\nEn route visit Devprayag, Rudraprayag, Karnaprayag – sacred river confluences\nArrive at Joshimath or Pipalkoti in the evening\nOvernight Stay: Joshimath / Pipalkoti"
    //         },
    //         {
    //             "day": 2,
    //             "title": "Joshimath / Pipalkoti – Badrinath – Rudraprayag / Srinagar",
    //             "description": "Joshimath / Pipalkoti – Badrinath – Rudraprayag / Srinagar (180–200 km total / 7–8 hrs)\nEarly morning drive to Badrinath (via Joshimath if staying in Pipalkoti)\nDarshan at Badrinath Temple, take a dip in Tapt Kund, visit Narad Kund\nVisit Mana Village, Vyas Gufa, Bhim Pul, and Saraswati River origin\nAfter sightseeing and darshan, drive back towards Rudraprayag/Srinagar\nOvernight Stay: Rudraprayag / Srinagar"
    //         },
    //         {
    //             "day": 3,
    //             "title": "Rudraprayag / Srinagar – Haridwar",
    //             "description": "Rudraprayag / Srinagar – Haridwar (160–180 km / 6–7 hrs)\nAfter breakfast, begin the drive back to Haridwar\nEn route enjoy views of Ganga Valley and small towns\nReach Haridwar by evening\nTour concludes with blessings of Lord Badri Vishal"
    //         }
    //     ],
    //     "inclusions": [
    //         "Transport by private vehicle from Haridwar to Haridwar.",
    //         "Accommodation on double/triple sharing basis.",
    //         "Breakfast and dinner during the trip.",
    //         "Driver allowances, tolls, parking, and fuel.",
    //         "Basic assistance during temple visit."
    //     ],
    //     "exclusions": [
    //         "Personal expenses, snacks, bottled water, tips.",
    //         "Special pooja arrangements or VIP darshan passes.",
    //         "Medical/emergency expenses.",
    //         "Anything not mentioned in 'Inclusions'."
    //     ],
    //     "things_to_carry": [],
    //     "best_time_to_visit": [],
    //     "why_choose_us": [
    //         "Local experts with years of Himalayan yatra experience.",
    //         "Comfortable and clean stays for a rejuvenating journey.",
    //         "Well-maintained vehicles and trained drivers.",
    //         "Personalized service and end-to-end yatra coordination.",
    //         "Transparent pricing and support for elderly/yatris."
    //     ]
    // },
]

export default tours;
